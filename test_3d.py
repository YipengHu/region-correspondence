...
# read test masks
masks_mov = torch.stack([torch.tensor(nib.load("./data/3d/test0_mask{}.nii.gz".format(idx)).get_fdata()) for idx in range(8)], dim=0).to(torch.bool)
masks_fix = torch.stack([torch.tensor(nib.load("./data/3d/test1_mask{}.nii.gz".format(idx)).get_fdata()) for idx in range(8)], dim=0).to(torch.bool)

paired_rois = PairedRegions(masks_mov=masks_mov, masks_fix=masks_fix, device=device)
ddf = paired_rois.get_dense_correspondence(transform_type='ddf', max_iter=int(2e4), lr=1e-3, w_ddf=10.0, verbose=True)
...
print("Saved ./data/3d/warped_mask*.nii.gz")
